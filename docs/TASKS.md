...
- 前端：送出成功畫面給編輯連結與複製鈕；讀到 `edit` 參數即存進 localStorage 並從網址列拿掉；修改模式重用回報表單，可改位置，另有撤回鈕；「我的回報」清單；卡片的「修改這筆回報」
...
- 狀態（2026-09-23）：本機端到端完成（送出、清空 localStorage 後打開編輯連結、修改、快照後退場、撤回、失效連結提示；補發工具對本機 30 筆假資料實跑並重跑確認不重發）。同日遠端套用 `0002_edit_token.sql` 並部署，正式補發 15 筆（線上測試點位的連結以 `GET` 驗證為 200，無密鑰為 404）。待做：補發的連結存進站方瀏覽器；真機跑一次「送出 → 另一個瀏覽器打開 → 修改 → 撤回」
...
- 「殘根未清」（處置為僅剩根部、僅剩主幹、樹穴填平）標為持續風險層
- 官方清冊消失但無回報層（依 M3 結果）
- 與受保護樹木疊合找高風險老樹群
