...
- 篩選：原因、處置、證據來源、資料來源、發現日期範圍，全部在前端對快照做。
...
- 選點時可貼上從地圖 app 複製的位置或輸入受保護樹木編號查位置，結果和 GPS 一樣只用來 flyTo，飛到 zoom 18；全部在本機解析、不送出（`web/src/report/lookup.ts`、`web/src/report/coordinates.ts`）。位置接受十進位度（可加括號，經緯順序顛倒時自動對調）、度分秒、Plus Code（完整碼，或後面接地名的簡碼，以收件範圍中心補回前 4 碼），超出收件範圍時只提示不移動。純數字視為受保護樹木編號，在已載入的 `trees.json` 裡找。網址與地址不支援，其他輸入一律提示可用的格式。查詢區預設收合，由 sheet 標題列「說明」左邊的「查詢」chip 展開，不記住狀態；填寫模式鎖定點位時與送出成功畫面不顯示。
